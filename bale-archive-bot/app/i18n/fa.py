...
def bot_added_ask_role(group_title: str) -> str:
    return (
        ...
        "مالک گروه باید بازو را مستقیماً به‌عنوان مدیر (ادمین) اضافه کند تا متن عادی "
        "و فوروارد بدون منشن دریافت شود.\n"
...
def bot_join_needs_direct_admin(group_title: str) -> str:
    return (
        f"بازو از «{group_title}» خارج شد چون دسترسی دریافت همه پیام‌های گروه را نداشت.\n"
        "مالک گروه باید بازو را مستقیماً از بخش «مدیران ← افزودن مدیر» اضافه کند؛ "
        "اضافه‌کردن به‌عنوان عضو برای آرشیو متن عادی و فوروارد کافی نیست."
    )


def research_owner_setup_done(group_title: str) -> str:
    return (
...
def research_set_done(group_title: str, bot_username: str | None = None) -> str:
    ...
    return (
        ...
        "این کار را مالک همان گروه انجام دهد تا دسترسی دریافت همه پیام‌ها ثبت شود.\n"
...
def research_need_admin(group_title: str, bot_username: str | None = None) -> str:
    ...
    return (
        ...
        "\n"
        "اگر پیام عادی یا فوروارد نمی‌رسد، مالک گروه بازو را یک‌بار حذف کند و از مسیر "
        "«اطلاعات گروه ← مدیران ← افزودن مدیر (ادمین)» مستقیماً دوباره اضافه کند.\n"
        "نقش رصد پاک نمی‌شود و بعد از افزودن درست، منشن لازم نیست."
    )
