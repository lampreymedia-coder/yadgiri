...
def bot_added_ask_role(group_title: str) -> str:
    return (
        ...
        "بعد از انتخاب رصد، ربات را در همان گروه ادمین کنید وگرنه پیام‌ها دیده نمی‌شوند.\n"
...
def _research_mention_example(bot_username: str | None) -> str:
    username = (bot_username or "نام‌کاربری‌ربات").lstrip("@")
    return f"@{username} یک متن آزمایشی"
...
def research_set_done(group_title: str, bot_username: str | None = None) -> str:
    return (
        ...
        "\n"
        "برای فعال‌شدن مطمئن در بله، بازو را از مسیر «اطلاعات گروه ← مدیران ← افزودن مدیر» "
        "مستقیماً به‌عنوان مدیر (ادمین) اضافه کنید.\n"
        "اگر بازو اول عضو بوده و بعداً مدیرش کرده‌اید ولی پیام عادی نمی‌رسد، "
        "یک‌بار حذفش کنید و از همان بخش مدیران دوباره اضافه کنید؛ نقش رصد پاک نمی‌شود.\n"
        "\n"
        "آزمایش فوری و مسیر جایگزین قطعی:\n"
        f"{_research_mention_example(bot_username)}\n"
        "اگر بله پیام عادی را تحویل ندهد، پیام منشن‌شده باز هم سؤال هشتگ را "
...
def research_need_admin(group_title: str, bot_username: str | None = None) -> str:
    return (
        ...
        "\n"
        "اگر با وجود ادمین بودن، پیام عادی نمی‌رسد، بازو را یک‌بار حذف کنید و از مسیر "
        "«اطلاعات گروه ← مدیران ← افزودن مدیر» مستقیماً دوباره اضافه کنید.\n"
        "نقش رصد پاک نمی‌شود.\n"
        "\n"
        "تا آن موقع این مسیر قطعی کار می‌کند:\n"
        f"{_research_mention_example(bot_username)}"
    )
...
def research_rejoined(group_title: str, bot_username: str | None = None) -> str:
    return (
        ...
        "حالا یک متن معمولی در گروه بفرستید؛ سؤال هشتگ باید در خصوصی فرستنده باز شود.\n"
        "\n"
        "مسیر جایگزین قطعی:\n"
        f"{_research_mention_example(bot_username)}"
    )
