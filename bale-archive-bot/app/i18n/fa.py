...
def admin_owner_post_sync_failed(short_id: str) -> str:
    return (
        "⚠️ همگام‌سازی جدول POST ناموفق بود\n"
        "\n"
        f"شناسه بایگانی: {short_id}\n"
        "بایگانی اصلی سالم است و ذخیره شده؛ فقط درج در جدول POST شکست خورد.\n"
        "جزئیات در لاگ سیستم ثبت شده است."
    )
...
ERR_UNKNOWN_COMMAND = (
    "این دستور را ندارم.\n"
    "\n"
    "از دکمه‌های پایین صفحه استفاده کنید، یا یکی از این دستورها را بفرستید:\n"
    "/help — نحوه کار\n"
    "/menu — دکمه‌های پایین صفحه\n"
    "/tags — هشتگ‌ها\n"
    "/my — ثبت‌های من\n"
    "/status — وضعیت ربات\n"
...
def start_owner_setup(name: str) -> str:
    ...
    return (
        ...
        "دکمه‌های پایین صفحه کار می‌کنند؛ برای راه‌اندازی گروه‌های آرشیو هم همین‌جا راهنما هست.\n"
        "\n"
...
MENU_HEADER = "دکمه‌های پایین صفحه منوی ربات‌اند. هر کدام همان لحظه جواب می‌دهد."
ADD_TO_GROUP_UNAVAILABLE = "شناسه کاربری ربات هنوز مشخص نیست. کمی بعد دوباره تلاش کنید."


def add_to_group_invite(url: str) -> str:
    return (
        "با این دکمه ربات را به گروه رصد یا آرشیو اضافه کنید.\n"
        "اول عضو کنید، بعد از فهرست اعضا ادمین کنید.\n"
        f"\n{url}"
    )
...
PANEL_HEADER = (
    "🛠 پنل مدیریت\nیکی از دکمه‌های همین پیام را بزنید تا گزارش یا فایل برایتان ارسال شود."
)
