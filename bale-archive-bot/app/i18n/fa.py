...
def research_set_done(group_title: str) -> str:
    return (
        ...
        "\n"
        "گام بعد — لازم است:\n"
        "داخل همان گروه، ربات را ادمین کنید (ارسال پیام کافی است).\n"
        "تا ادمین نباشد، بله پیام اعضای گروه را به ربات نمی‌دهد "
        "و سؤال هشتگ در خصوصی نمی‌آید.\n"
        "\n"
        "بعد از ادمین شدن، یک متن معمولی در گروه بفرستید "
        "(ربات را منشن نکنید). سؤال هشتگ همان‌جا در خصوصی فرستنده باز می‌شود."
    )


def research_need_admin(group_title: str) -> str:
    return (
        f"گروه «{group_title}» از قبل رصد است.\n"
        "\n"
        "اگر پیام می‌فرستید و سؤال هشتگ نمی‌آید، ربات در آن گروه ادمین نیست.\n"
        "ربات را ادمین کنید، بعد یک متن جدید بفرستید. "
        "منشن و /start لازم نیست."
    )
